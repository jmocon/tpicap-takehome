...
**Prompt:** "enhance our layout [mockup image] do something like this"

**Outcome:** A reference mockup rather than a written brief, so the first step was reading the image against the running app and listing what actually differed — icons throughout, a full-bleed nav with underline tabs, taller uniform controls, card-framed table, ticker avatars, a status pill — rather than "restyling" by feel. Implemented as presentation-only changes plus four small shared components (a hand-rolled 17-glyph inline SVG icon set instead of an icon dependency, a `SelectField` shell around the native `<select>`, a hash-coloured `SymbolBadge`, and a shared chart-card header). Verified in a real headless browser, not just by unit tests, which is what caught two things the test suite couldn't: the new `.trade-table td` colour rule silently out-specified `.pnl-positive`/`.pnl-negative` and killed P&L colouring on the Positions page, and the By Symbol page was logging "Maximum update depth exceeded". The second one turned out to be **pre-existing** — confirmed by re-running the same probe against a stashed checkout before claiming credit or blame — caused by `useTrades({})` passing a new object literal each render into a `useCallback([query])`, i.e. an infinite fetch loop. Fixed with a module-level constant.

---

**Prompt:** "im getting this error [`WARN WS connection rejected: missing or invalid token`]"

**Outcome:** A one-line log excerpt with no reproduction steps, so the work was tracing the token from where it's minted to where it's checked before changing anything — which is what kept the fix off the obvious-but-wrong target. The log line comes from the backend refusing a WS upgrade, and the tempting read is that the WS auth check is broken. It isn't: signing and verification share one secret, and the check is right. The defect was that the frontend could never *recover* from a rejection — `tokenStorage.load()` restored any token without checking `exp`, `RequireAuth` only tests for a `user` object, so a session hours past its 8h expiry still mounted the blotter, and the socket's reconnect handler retried every 2s regardless of *why* it closed. The log spam was an unauthenticated retry loop with no UI signal, and REST 401s were being swallowed the same way. Fixed by making a rejected token terminal: stop reconnecting on close code 4401, drop expired tokens at load, treat a 401 as end-of-session (except on `/auth/login`, where it just means wrong password), and route all of that through one `onAuthFailure` hook in the framework-agnostic token module so the HTTP client and socket don't have to import React. Four tests added for the behaviours that had none.
